...
const cloudMessage =
  "***DISCOUNT*** Having trouble setting up WolfBot on your local machine? Try the cloud version at https://wolfbot.org with a 30% discount with this code: gitsource7393";
try {
...
if (argv.p) {
  ...
  // allow a main and test instance run together
  nconf.set("port", (nconf.get("port") ? nconf.get("port") : 4000) + 2);
  nconf.set(
    "tlsPort",
    (nconf.get("tlsPort") ? nconf.get("tlsPort") : 4000) + 4
  );
}
if (!nconf.get("port")) nconf.set("port", nconf.get("portDef"));
if (!nconf.get("tlsPort")) nconf.set("tlsPort", nconf.get("tlsPortDef"));
if (!argv.trader) nconf.set("trader", "RealTimeTrader");
...
if (argv.noBrowser) logger.info(cloudMessage);
...
const updateHandler =
  argv.noUpdate === true ? null : require("./src/updateHandler");
...
let checkAuth = (apiKey, res) => {
  if (!nconf.get("apiKeys")) return;
  let keys = nconf.get("apiKeys");
  if (!apiKey || keys[apiKey] !== true) {
    let output = {
      error: true,
      errorTxt:
        'Invalid api key. Please add a parameter "apiKey" to your GET/POST request.',
      key: apiKey,
...
dispatcher.beforeFilter(/\//, (req, res, chain) => {
  ...
  if (
    !nconf.get("debug") &&
    req.url.match(/\.(ts|js\.map)(\?.*)?$/i) !== null
  ) {
    // prevent access to typescript files
...
dispatcher.onPost("/state/", async (req, res) => {
  ...
  if (checkPostAuth(req.formFields, res) === false) return;
...
dispatcher.onPost("/telegram/", (req, res) => {
  ...
  if (json && json.message) Controller.addTelegramMessage(json.message);
...
dispatcher.onGet("/dl/", (req, res) => {
  if (checkAuth(req.params.apiKey, res) === false) return;
...
dispatcher.onPost("/tradebook/", (req, res) => {
  if (checkPostAuth(req.formFields, res) === false) {
    if (!req.params || checkAuth(req.params.apiKey, res) === false) return;
  }
...
const indexPage = fs.readFileSync(
  path.join(utils.appDir, "public", "index.html")
);
dispatcher.onError((req, res) => {
  if (
    ...
    res.writeHead(404, { "Content-Type": "text/plain; charset=UTF-8" });
    res.end("Not found: " + req.url);
  } else res.end(indexPage); // send all routes to the client. the client will show 404
});
...
let handleRequest = (request, response) => {
  try {
    ...
    if (
      nconf.get("httpsOnly") === true &&
      nconf.get("protocol") === "https://"
    ) {
      ...
      let redirect =
        "https://" + urlParts.hostname + ":" + nconf.get("tlsPort"); // browsers won't display port 443
...
if (helper.getFirstApiKey()) startPath += helper.getFirstApiKey();
else startPath += "YOUR_API_KEY_FROM_CONFIG";
...
server.listen(nconf.get("port"), () => {
  if (nconf.get("serverConfig:premium") === false)
    logger.info(
      "Server is listening on %s%s:%s%s ",
      "http://",
      os.hostname(),
      nconf.get("port"),
      startPath
    ); // TODO why is our tail() cutting the last char of first line?
});
  ...
  let credentials = {
    key: fs
      .readFileSync(
        path.join(
          utils.appDir,
          nconf.get("serverDataDir"),
          nconf.get("projectName") + ".key"
        )
      )
      .toString(),
    cert: fs
...
if (updateHandler !== null) {
  updateHandler.runUpdater(() => {
    // will restart the app or fire this callback
    if (argv.update) {
      process.exit(0);
      return;
    }
...
} else Controller.start(mainServer);
