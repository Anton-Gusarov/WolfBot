...
export abstract class ServerSocketPublisher {
  ...
  public onSubscription(
    ...
    initialRequest: http.IncomingMessage
  ): void {
  ...
  public handleData(
    data: any,
    clientSocket: ClientSocketOnServer,
    initialRequest: http.IncomingMessage
  ): void {
    if (data.action) {
      if (data.action === "sub") {
  ...
  protected send(
    ws: ClientSocketOnServer,
    data: any,
    options?: ServerSocketSendOptions
  ) {
    return this.serverSocket.send(
      ws,
      ServerSocket.stringify(this.opcode, data, ws.jsonFormat),
      options
    );
    //return this.serverSocket.send(ws, ws.stringify(this.opcode, data), options)
  }

  protected sendWithOpcode(
    ws: ClientSocketOnServer,
    ...
    return this.serverSocket.send(
      ws,
      ServerSocket.stringify(opcode, data, ws.jsonFormat),
      options
    );
  }
  ...
  protected publish(
    data: any,
    excludeClients = new Set<ClientSocketOnServer>(),
    options?: ServerSocketSendOptions
  ) {
    return new Promise<void>((resolve, reject) => {
      if (this.clientSocket !== undefined) {
        this.clientSocket.send([this.opcode, data], options, (err) => {
          if (err) logger.error("Error sending WebSocket data to client", err);
        });
        return;
      }
      let clients = [],
      ...
      for (let client of this.subscribers) {
        if (excludeClients.has(client[0]) === false) {
          if (client[0].jsonFormat === "EJSON") clientsEJSON.push(client[0]);
          else clients.push(client[0]);
        }
      }
      ...
      this.serverSocket
        .broadcastTo(
          clients.length !== 0
            ? ServerSocket.stringify(this.opcode, data, "JSON")
            : "",
          clients,
          options
        )
        .then(() => {
          return this.serverSocket.broadcastTo(
            clientsEJSON.length !== 0
              ? ServerSocket.stringify(this.opcode, data, "EJSON")
              : "",
            clientsEJSON,
            options
          );
  ...
  protected schedulePing(clientSocket: ClientSocketOnServer) {
    setTimeout(() => {
      this.sendWithOpcode(clientSocket, WebSocketOpcode.PING, {
        ping: Date.now(),
      }).then((success) => {
        if (success !== false) this.schedulePing(clientSocket);
      });
...
export interface ServerSocketSendOptions {
  // from ws.send()
  mask?: boolean;
  binary?: boolean;
}
...
export class ServerSocket extends WebSocket.Server {
  ...
  public static stringify(
    ...
    if (AppJSONFormat === "JSON") return JSON.stringify([opcode, data]);
    return utils.EJSON.stringify([opcode, data]);
  }
  ...
  public send(
    ws: /*WebSocket*/ ClientSocketOnServer,
    data: any,
    options?: ServerSocketSendOptions
  ) {
    return new Promise<boolean>((resolve, reject) => {
      ws.send(data, options, (err) => {
        if (err) {
          ...
          } else logger.error("Error sending WebSocket data", err);
  ...
  public broadcast(
    data: any,
    excludeClients = new Set<WebSocket>(),
    options?: ServerSocketSendOptions
  ) {
    return new Promise<void>((resolve, reject) => {
      let sendOps = [];
      this.clients.forEach((client) => {
        if (
          ...
          excludeClients.has(client) === false
        ) {
          sendOps.push(
            new Promise((resolve, reject) => {
              client.send(data, options, (err) => {
                if (err)
                  logger.error(
                    "Error broadcasting WebSocket data to client",
                    err
                  );
  ...
  protected waitForConnections() {
    ...
    this.on("connection", (ws: ClientSocketOnServer, request) => {
      ...
      if (utils.objects.isEmpty(keys) === false) {
        // otherwise no API keys configured (public access for everybody)
        // if (!params["apiKey"] || keys[params["apiKey"]] !== true) {
        //   logger.warn(
        //     "Refusing WebSocket connection without authentication on %s from %s",
        //     request.url,
      ...
      // TODO instantiate subclass?
      else
        logger.warn(
          "Property 'id' is already defined for incoming WebSocket connection",
          ws.id
        );
      ws.jsonFormat =
      ...
      ws.on("message", (message) => {
        try {
          if (typeof message !== "string")
            return logger.warn(
              "Received WebSocket data with unexpected type %s",
              typeof message
            );
          const messageData =
            AppJSONFormat === "JSON"
              ? JSON.parse(message)
              : utils.EJSON.parse(message);
          let receiver = this.receivers.get(messageData[0]);
          if (receiver !== undefined)
            receiver.handleData(messageData[1], ws, request);
          else
            logger.warn(
              "Received WebSocket data without corresponding receiver. Opcode %s",
              messageData[0]
            );
        } catch (err) {
          logger.error(
            "Error parsing WebSocket message in %s:",
            this.className,
            err
          );
      ...
      ws.on("close", (code, message) => {
        logger.verbose(
          "WebSocket connection closed with code %s: %s",
          code,
          message
        );
        this.removeWebsocketClientFromAllReceivers(ws);
  ...
  protected removeWebsocketClientFromAllReceivers(ws: ClientSocketOnServer) {
    for (let receiver of this.receivers) receiver[1].removeSubscriber(ws);
  }

  protected sendErrorAndClose(
    ws: ClientSocketOnServer,
    ...
    return this.sendAndClose(
      ws,
      ServerSocket.stringify(
        WebSocketOpcode.FATAL_ERROR,
        { err: error },
        ws.jsonFormat
      ),
      options
